    ...
    try {
        ...
        if (error.code === 11000) {
            throw new CustomError(
                ...
                "Project owner cannot create two project of same name"
            );
        }
        throw new CustomError(
...
const getProjects = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(200).json(
        new ApiResponse(ResponseStatus.Success, projects, "Projects returned")
    );
...
const getUserRole = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!projectId || !userId) {
        throw new CustomError(400, "Project ID or User ID is missing");
    }

    // Look for project membership
    ...
    if (!projectMember) {
        throw new CustomError(400, " User is not a member of the project");
    }
