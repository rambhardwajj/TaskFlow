...
const updateTask = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (email !== undefined)
        updatePayload.assignedTo = assignedTo._id as string;
...
const deleteTask = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(ResponseStatus.Success).json(
            new ApiResponse(ResponseStatus.Success, null, "Task deleted ")
        );
    } catch (error: any) {
...
const addAttachments = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existingAttachments + newAttachments > 5) {
        throw new CustomError(400, "Attachment limit exceeded.");
    }
...
const getSubTasks = asyncHandler(async (req, res) => {
    ...
    const subTasks = await SubTask.aggregate([
        {
            $match: {
                task: new mongoose.Types.ObjectId(taskId as string),
            },
        },
    ]);
    ...
    res.status(200).json(
        new ApiResponse(200, subTasks, "Subtasks retrieved successfully")
    );
});
